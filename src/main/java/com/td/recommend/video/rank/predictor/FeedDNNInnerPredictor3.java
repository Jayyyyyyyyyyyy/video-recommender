...
import com.td.recommend.video.rank.featureextractor.FeedDNNFeatureExtractor3;
 ...
 */
public class FeedDNNInnerPredictor3 implements InnerPredictor {
    private static final Logger LOG = LoggerFactory.getLogger(FeedDNNInnerPredictor3.class);
    ...
    private DNNFeatureExtractor featureExtractor = FeedDNNFeatureExtractor3.getInstance();

    public FeedDNNInnerPredictor3(VideoRecommenderContext recommendContext, FeatureDumperSampler featureDumperSampler, int predictTimeoutMs) {
        this.recommendContext = recommendContext;
        this.bucketPredictModel = ReloadableDNNModel.feed3;
    ...
    public FeedDNNInnerPredictor3(VideoRecommenderContext recommendContext, FeatureDumperSampler featureDumperSampler) {
    ...
    public Optional<PredictResult> predict(PredictItems<DocItem> predictItems, Items queryItems, String predictId) {
        ...
        taggedMetricRegistry.meter("usernews.dnn3.predict.qps").mark();
        Context predictTime = null;
        if (!recommendContext.isDebug()) {
            predictTime = taggedMetricRegistry.timer("usernews.dnn3.predict.latency").time();
        }
...
//        LOG.info("userFeatures : [{}]", JSON.toJSONString(featureJson));
        ...
        try {
            Context userFeature = taggedMetricRegistry.timer("usernews.dnn3.predict.userFeature.latency").time();
            ...
            userFeature.stop();
            DNNFeatureMonitor.getInstance().asynMonitor(featureConfig, docIrrelevantBucketizedFeatures, 0, "dnn3", recommendContext);
            ...
            taggedMetricRegistry.histogram("usernews.dnn3.predict.docIrrelevantIdFeatures.failrate").update(0);
        } catch (Exception e) {
            taggedMetricRegistry.histogram("usernews.dnn3.predict.docIrrelevantIdFeatures.failrate").update(100);
...
//                        printList.add(ImmutableMap.of(predictTaskItem.getDocId(), predictTaskItem.getFeature()));
                    ...
                    taggedMetricRegistry.histogram("usernews.dnn3.predict.taskfailrate").update(0);
                } catch (InterruptedException e) {
                    taggedMetricRegistry.histogram("usernews.dnn3.predict.taskfailrate").update(100);
...
//        });
        ...
        float[][] predictArray;
        try {
            predictArray = dnnModel.predict(dumpInfoList, docIrrelevantIdFeatures, featureConfig, mtlConfigV2.size(), "dnn3");
            if (predictArray.length == dumpInfoList.size()) {
                taggedMetricRegistry.histogram("usernews.dnn3.predict.score.length").update(0);
            } else {
                taggedMetricRegistry.histogram("usernews.dnn3.predict.score.length").update(100);
            }
            taggedMetricRegistry.histogram("usernews.dnn3.predict.score").update(0);
        } catch (Exception e) {
            taggedMetricRegistry.histogram("usernews.dnn3.predict.score").update(100);
...
//            taggedMetricRegistry.meter("usernews.dnn3.mtlBucket." + mtlBucket + ".qps").mark();
//        }
        ...
        DNNPredictMonitor.getInstance().asyncMonitor(predictArray, mtlConfigV2, "dnn3");
        DNNPredictMonitor.getInstance().asyncMonitor(scoreMonitor, "dnn3");
        ...
        if (scoreMap.size() < predictItems.getItems().size()) {
            taggedMetricRegistry.histogram("usernews.dnn3.predict.lessrate").update(100);
        } else {
            taggedMetricRegistry.histogram("usernews.dnn3.predict.lessrate").update(0);
        }
    ...
    private String mtlBucket(Map<String, Map<String, TVariance>> userVarianceMap) {
        ...
        if (userVarianceMap == null || userVarianceMap.isEmpty()) {
            if (recommendContext.getRecommendRequest().getAppId().equals("t01")) {
                taggedMetricRegistry.histogram("usernews.dnn3.user.profile.loss.rate").update(100);
            }
            return "newcomer";
        }
        ...
        taggedMetricRegistry.histogram("usernews.dnn3.user.profile.loss.rate").update(0);
        double negCnt = overallStats.negCnt;
        if (negCnt <= 5) {
            taggedMetricRegistry.histogram("usernews.dnn3.user.profile.click.less.5.rate").update(100);
            return "newcomer";
        }
        taggedMetricRegistry.histogram("usernews.dnn3.user.profile.click.less.5.rate").update(0);
