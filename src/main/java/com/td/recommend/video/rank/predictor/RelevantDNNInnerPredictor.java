...
import com.td.recommend.video.rank.featureextractor.RelevantDNNFeatureExtractor;
 ...
 */
public class RelevantDNNInnerPredictor implements InnerPredictor {
    private static final Logger LOG = LoggerFactory.getLogger(RelevantDNNInnerPredictor.class);
    ...
    private DNNFeatureExtractor featureExtractor = RelevantDNNFeatureExtractor.getInstance();

    public RelevantDNNInnerPredictor(VideoRecommenderContext recommendContext, FeatureDumperSampler featureDumperSampler, int predictTimeoutMs) {
        this.recommendContext = recommendContext;
        this.bucketPredictModel = ReloadableDNNModel.rlvt;
    ...
    public RelevantDNNInnerPredictor(VideoRecommenderContext recommendContext, FeatureDumperSampler featureDumperSampler) {
    ...
    public Optional<PredictResult> predict(PredictItems<DocItem> predictItems, Items queryItems, String predictId) {
        ...
        taggedMetricRegistry.meter("rlvt.dnn.predict.qps").mark();
        Context predictTime = taggedMetricRegistry.timer("rlvt.dnnpredict.latency").time();
        ...
        RelevantDNNFeatureExtractor.extractStaticDocFeatures(featureJson, currentDocItem, "t_");
...
//        LOG.info("userFeatures : [{}]", JSON.toJSONString(featureJson));
        ...
        try {
            Context userFeature = taggedMetricRegistry.timer("rlvt.dnnpredict.userFeature.latency").time();
            ...
            userFeature.stop();
            DNNFeatureMonitor.getInstance().asynMonitor(featureConfig, docIrrelevantBucketizedFeatures, 0, "rlvtdnn", recommendContext);
            ...
            taggedMetricRegistry.histogram("rlvt.dnnpredict.docIrrelevantIdFeatures.failrate").update(0);
        } catch (Exception e) {
            taggedMetricRegistry.histogram("rlvt.dnnpredict.docIrrelevantIdFeatures.failrate").update(100);
            LOG.error("rlvt docIrrelevantIdFeatures extract failed", e);
        }
...
//                        printList.add(ImmutableMap.of(predictTaskItem.getDocId(), predictTaskItem.getFeature()));
                    ...
                    taggedMetricRegistry.histogram("rlvt.dnnpredict.taskfailrate").update(0);
                } catch (InterruptedException e) {
                    taggedMetricRegistry.histogram("rlvt.dnnpredict.taskfailrate").update(100);
                    LOG.error("rlvt batch predict is interrupted!", e);
                } catch (ExecutionException e) {
                    taggedMetricRegistry.histogram("rlvt.dnnpredict.taskfailrate").update(100);
                    LOG.error("rlvt batch predict execution failed!", e);
                }
            } else {
                taggedMetricRegistry.histogram("rlvt.dnnpredict.taskfailrate").update(100);
                LOG.error("rlvt batch predict is canceled!");
            }
        }
        // ******
...
//        });
        ...
        float[][] predictArray;
        try {
            predictArray = dnnModel.predict(dumpInfoList, docIrrelevantIdFeatures, featureConfig, mtlConfigV2.size(), "rlvtdnn");
            if (predictArray.length == dumpInfoList.size()) {
                taggedMetricRegistry.histogram("rlvt.dnnpredict.score.length").update(0);
            } else {
                taggedMetricRegistry.histogram("rlvt.dnnpredict.score.length").update(100);
            }
            taggedMetricRegistry.histogram("rlvt.dnnpredict.score").update(0);
        } catch (Exception e) {
            taggedMetricRegistry.histogram("rlvt.dnnpredict.score").update(100);
            LOG.error("rlvt dnn predict failed", e);
            return Optional.empty();
        }
        String mtlBucket = mtlBucket(userVarianceMap);
        if (recommendContext.getRecommendRequest().getAppId().equals("t01")) {
            taggedMetricRegistry.meter("rlvt.dnn.mtlBucket." + mtlBucket + ".qps").mark();
        }
        ...
        for (int i = 0; i < predictArray.length; i++) {
            try {
                ...
                LOG.error("rlvt dnn felib predict error", e);
            }
        }
        DNNPredictMonitor.getInstance().asyncMonitor(predictArray, mtlConfigV2, "rlvtdnn");
        DNNPredictMonitor.getInstance().asyncMonitor(scoreMonitor, "rlvtdnn");
        LOG.info("rlvt dnn predictItems length : {} || dumpInfoList length : {} || predictList length : {}", predictItems.getSize(), dumpInfoList.size(), predictArray.length);
        if (scoreMap.size() < predictItems.getItems().size()) {
            taggedMetricRegistry.histogram("rlvt.dnnpredict.lessrate").update(100);
        } else {
            taggedMetricRegistry.histogram("rlvt.dnnpredict.lessrate").update(0);
        }
        ...
        for (PredictItem<DocItem> predictItem : predictItems) {
            ...
            for (int i = 0; i < mtlConfigV2.size(); i++) {
                try {
                    ...
                    LOG.error("rlvt dnn felib get taskName error", e);
                }
            }
    ...
    static class PredictTask implements Callable<List<PredictTaskItem>> {
        ...
        public List<PredictTaskItem> call() {
            ...
            if (nullCount > 0) {
                LOG.error("rlvt Found null docItem count={}, items size={}", nullCount, predictItems.size());
            }
            for (PredictItem<DocItem> predictItem : predictItems) {
