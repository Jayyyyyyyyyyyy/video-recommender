...
public class FeedDNNFeatureExtractor3 implements DNNFeatureExtractor {
    private static final Logger LOG = LoggerFactory.getLogger(FeedDNNFeatureExtractor3.class);
    private static final FeedDNNFeatureExtractor3 instance = new FeedDNNFeatureExtractor3();

    public static FeedDNNFeatureExtractor3 getInstance() {
        return instance;
    }
    ...
    public JSONObject extract(PredictItem<DocItem> predictItem, FeatureConfig featureConfig, Vocabulary vocabulary, Buckets buckets, JSONObject docIrrelevantRawJSON, JSONObject docIrrelevantRawFeatures, JSONObject docIrrelevantBucketizedFeatures, VideoRecommenderContext recommendContext) {
        ...
        Timer.Context singleExtract = taggedMetricRegistry.timer("usernews.dnn3.predict.single.itemfeature.extract.latency").time();
        ...
        try {
            ...
            if (r < 0.01) {
                DNNFeatureMonitor.getInstance().asynMonitor(featureConfig, docRelevantBucketizedFeatures, 1, "dnn3", recommendContext);
            }
