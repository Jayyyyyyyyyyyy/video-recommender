...
public class FeedDNNFeatureExtractor implements DNNFeatureExtractor {
    private static final Logger LOG = LoggerFactory.getLogger(FeedDNNFeatureExtractor.class);
    private static final FeedDNNFeatureExtractor instance = new FeedDNNFeatureExtractor();

    public static FeedDNNFeatureExtractor getInstance() {
        return instance;
    }
    ...
    public JSONObject extract(PredictItem<DocItem> predictItem, FeatureConfig featureConfig, Vocabulary vocabulary, Buckets buckets, JSONObject docIrrelevantRawJSON, JSONObject docIrrelevantRawFeatures, JSONObject docIrrelevantBucketizedFeatures, VideoRecommenderContext recommendContext) {
        ...
        Timer.Context singleExtract = taggedMetricRegistry.timer("usernews.dnn.predict.single.itemfeature.extract.latency").time();
        ...
        try {
            ...
            if (r < 0.01) {
                DNNFeatureMonitor.getInstance().asynMonitor(featureConfig, docRelevantBucketizedFeatures, 1, "dnn", recommendContext);
            }
