...
public class RelevantDNNFeatureExtractor implements DNNFeatureExtractor {
    private static final Logger LOG = LoggerFactory.getLogger(RelevantDNNFeatureExtractor.class);
    private static final RelevantDNNFeatureExtractor instance = new RelevantDNNFeatureExtractor();

    public static RelevantDNNFeatureExtractor getInstance() {
        return instance;
    }
    ...
    public JSONObject extract(PredictItem<DocItem> predictItem, FeatureConfig featureConfig, Vocabulary vocabulary, Buckets buckets, JSONObject docIrrelevantRawJSON, JSONObject docIrrelevantRawFeatures, JSONObject docIrrelevantBucketizedFeatures, VideoRecommenderContext recommendContext) {
        ...
        if (docItem != null) {
            extractProbeDyns(docItem, recommendContext.getRlvtdyns(), featureJson);
        ...
        accLatestTimestampFeature(featureJson, docIrrelevantRawJSON);
        Timer.Context singleExtract = taggedMetricRegistry.timer("rlvt.dnn.predict.single.itemfeature.extract.latency").time();
        ...
        try {
            ...
            if (r < 0.01) {
                DNNFeatureMonitor.getInstance().asynMonitor(featureConfig, docRelevantBucketizedFeatures, 1, "rlvtdnn", recommendContext);
            }
    ...
    private void extractProbeDyns(DocItem item, Map<String, Map<String, Map<String, Number>>> rlvtdyns, JSONObject featureJson) {
        if (rlvtdyns != null && !rlvtdyns.isEmpty()) {
            String vid = item.getId();
            List<String> exclude = ImmutableList.of("impr", "p1mCvr", "p3mCvr", "p4mCvr", "p5mCvr");
            rlvtdyns.forEach((type, value) -> {
                if (value != null) {
                    Map<String, Number> dynsItem = value.get("vid_" + vid);
                    if (dynsItem != null && !dynsItem.isEmpty()) {
                        dynsItem.forEach((jsonKey, jsonValue) -> {
                            if (!exclude.contains(jsonKey)) {
                                featureJson.put("probeDyns-" + type + "-vid-" +  jsonKey, jsonValue);
                            }
                        });
                    }
                }
