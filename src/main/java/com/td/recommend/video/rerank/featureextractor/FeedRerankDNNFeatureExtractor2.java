...
public class FeedRerankDNNFeatureExtractor2 implements RerankDNNFeatureExtractor {
    private static final Logger LOG = LoggerFactory.getLogger(FeedRerankDNNFeatureExtractor2.class);
    private static final FeedRerankDNNFeatureExtractor2 instance = new FeedRerankDNNFeatureExtractor2();

    public static FeedRerankDNNFeatureExtractor2 getInstance() {
        return instance;
    }
...
//        featureJson.put("uiTags", tags);
        Timer.Context singleExtract = taggedMetricRegistry.timer("video-recommend.rerank.dnn2.single.item.feature.extract.latency").time();
        ...
        try {
            ...
            if (r < 0.01) {
                RerankDNNFeatureMonitor.getInstance().asynMonitor(featureConfig, docRelevantBucketizedFeatures, 1, "dnn2", recommendContext);
            }
