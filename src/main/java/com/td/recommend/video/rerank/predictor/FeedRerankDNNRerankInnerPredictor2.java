...
import com.td.recommend.video.rerank.featureextractor.FeedRerankDNNFeatureExtractor2;
 ...
 */
public class FeedRerankDNNRerankInnerPredictor2 implements RerankInnerPredictor {
    private static final Logger LOG = LoggerFactory.getLogger(FeedRerankDNNRerankInnerPredictor2.class);

    private final VideoRecommenderContext recommendContext;
    private final ReloadableRerankDNNModel bucketPredictModel;
    private RerankDNNFeatureExtractor featureExtractor = FeedRerankDNNFeatureExtractor2.getInstance();

    public FeedRerankDNNRerankInnerPredictor2(VideoRecommenderContext recommendContext) {
        this.recommendContext = recommendContext;
        this.bucketPredictModel = ReloadableRerankDNNModel.feed2;
    }
    ...
    public List<TaggedItem<PredictItem<DocItem>>> predict(List<TaggedItem<PredictItem<DocItem>>> taggedItems) {
        ...
        taggedMetricRegistry.meter("video-recommend.rerank.dnn2.predict.qps").mark();
        Context predictTime = null;
        if (!recommendContext.isDebug()) {
            predictTime = taggedMetricRegistry.timer("video-recommend.rerank.dnn2.predict.latency").time();
        }
...
//        rankList.sort((o1, o2) -> -Double.compare(o1.getItem().getScore(), o2.getItem().getScore()));
        ...
        try {
            Context userFeature = taggedMetricRegistry.timer("video-recommend.rerank.dnn2.user.feature.extract.latency").time();
            ...
            userFeature.stop();
            RerankDNNFeatureMonitor.getInstance().asynMonitor(featureConfig, docIrrelevantBucketizedFeatures, 0, "dnn2", recommendContext);
        ...
        try {
            ...
            if (predictArray.length == topN) {
                taggedMetricRegistry.histogram("video-recommend.rerank.dnn2.score.failure.rate").update(0);
            } else {
                taggedMetricRegistry.histogram("video-recommend.rerank.dnn2.score.failure.rate").update(100);
            }
        } catch (Exception e) {
            taggedMetricRegistry.histogram("video-recommend.rerank.dnn2.score.failure.rate").update(100);
        ...
        RerankDNNPredictMonitor.getInstance().asyncMonitor(predictArray, mtlConfigV3, "dnn2", recommendContext.getModel());
        RerankDNNPredictMonitor.getInstance().asyncMonitor(scoreMonitor, "dnn2");
        if (!recommendContext.isDebug()) {
            predictTime.stop();
        }
        return taggedItems;
