...
public class FeedRerankDNNFeatureExtractor implements RerankDNNFeatureExtractor {
    private static final Logger LOG = LoggerFactory.getLogger(FeedRerankDNNFeatureExtractor.class);
    private static final FeedRerankDNNFeatureExtractor instance = new FeedRerankDNNFeatureExtractor();

    public static FeedRerankDNNFeatureExtractor getInstance() {
        return instance;
    }
...
//        featureJson.put("uiTags", tags);
        Timer.Context singleExtract = taggedMetricRegistry.timer("video-recommend.rerank.dnn.single.item.feature.extract.latency").time();
        ...
        try {
            ...
            if (r < 0.01) {
                RerankDNNFeatureMonitor.getInstance().asynMonitor(featureConfig, docRelevantBucketizedFeatures, 1, "dnn", recommendContext);
            }
