 ...
 */
public class FollowInvalidItemFilterPreprocessor implements ItemsProcessor<PredictItems<DocItem>> {
    private static final Logger logger = LoggerFactory.getLogger(FollowInvalidItemFilterPreprocessor.class);
    private HashMap<String, Long> filterReasonMap = new HashMap<>();
    private static final Set<Integer> ctypes = ImmutableSet.of(101, 102, 103, 105, 106, 107);
    private static final Set<Integer> stageSet = ImmutableSet.of(6, 7, 8, 10);
    private VideoRecommenderContext recommendContext;

    public FollowInvalidItemFilterPreprocessor(VideoRecommenderContext recommendContext) {
    ...
    public PredictItems<DocItem> process(PredictItems<DocItem> items) {
        ...
        taggedMetricRegistry.histogram("follow.invalidItem.filtercount").update(items.getSize() - predictItemList.size());
