...
public class VideoRecommendContextBuilder {
    ...
    public VideoRecommenderContext build(RecommendRequest recommendRequest, boolean isDebug) {
        ...
        if (recommendRequest.getAppId().equals("t02") && buckets.contains("wxapp_implicit_recall-yes")) {
    ...
    public static void main(String[] argv) {
        String text = RedisClientSingleton.swipe.get("6EBB522D-0C89-453E-8525-00D7AF76A1CE");
        System.out.println(text);
        List<DynamicDumpInfo.Swipe> diu = JSON.parseArray(text, DynamicDumpInfo.Swipe.class);
        diu.get(0).getVideos().get(0).setVmp3("");
        System.out.println(JSON.toJSONString(diu));
        Set<String> collect = Arrays.asList("a", "a", "b").stream().collect(Collectors.toSet());
        System.out.println(collect);
//        List<DynamicDumpInfo.VidDuration> xx = parseClickedDurationHistory("866959037815461");
//
//        List<String> clicketd = ClickHistory.getInstance().clicked("866959037815461");
        System.out.println(ThreadLocalRandom.current().nextDouble());
        Set<String> buckets = RecommendContextBuilderHelper.buildBuckets("867076044199919", new RecommendRequest());
        buckets.stream().forEach(x -> System.out.println(x));
