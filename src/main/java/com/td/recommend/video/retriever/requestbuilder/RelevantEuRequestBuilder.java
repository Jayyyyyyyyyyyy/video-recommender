...
public class RelevantEuRequestBuilder implements RequestBuilder<RetrieveKey, HttpRequest> {
    private static final String REQUET_URI = "/recall/rlvt_eu";
    private static final Logger LOG = LoggerFactory.getLogger(com.td.recommend.requestbuilder.InterestRequestBuilder.class);
    private int num = 200;

    public RelevantEuRequestBuilder() {

    }

    public RelevantEuRequestBuilder(Config config) {
    ...
    public CompletableFuture<Optional<HttpRequest>> apply(RetrieveKey retrieveKey) {
        ...
        if (retrieveKey.getType().equals("vsleep_eu_rlvt")) {
