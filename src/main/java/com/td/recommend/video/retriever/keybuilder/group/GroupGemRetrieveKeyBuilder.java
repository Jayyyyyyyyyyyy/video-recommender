...
public class GroupGemRetrieveKeyBuilder implements RetrieveKeyBuilder {
    private VideoRecommenderContext recommendContext;
    private static final int MAX_COUNT = 50;
    private static final int MAX_ELAPSED_SECONDS = 60 * 60 * 6;

    public GroupGemRetrieveKeyBuilder(VideoRecommenderContext recommendContext) {
    ...
    public void build(UserItem userItem, RetrieveKeyContext retrieveKeyContext) {
        ...
        for (String docId : recentClicked) {
            ...
            retrieveKey.setType(RetrieverType.ggem.name())
                    .setAlias(RetrieverType.ggem.alias())
                    .setScore(1.0)
                    .setKey(docId)
