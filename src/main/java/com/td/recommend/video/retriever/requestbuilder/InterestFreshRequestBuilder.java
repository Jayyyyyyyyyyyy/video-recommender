 ...
 */
public class InterestFreshRequestBuilder implements RequestBuilder<RetrieveKey, HttpRequest> {
    private static String REQUET_URI = "/recall/interest_fresh";
    private static final Logger LOG = LoggerFactory.getLogger(com.td.recommend.requestbuilder.InterestRequestBuilder.class);
    private int num = 200;

    public InterestFreshRequestBuilder() {

    }

    public InterestFreshRequestBuilder(Config config) {
    ...
    public CompletableFuture<Optional<HttpRequest>> apply(RetrieveKey retrieveKey) {
        ...
        if(retrieveKey.getIhf().equals(String.valueOf(Ihf.VSHOWDANCE_FEED.id()))){
            ...
            requestParams.put("ctype", "103,105,107,109");
        }
        else{
