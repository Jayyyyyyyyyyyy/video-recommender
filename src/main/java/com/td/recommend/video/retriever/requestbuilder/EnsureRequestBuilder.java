 ...
 */
public class EnsureRequestBuilder implements RequestBuilder<RetrieveKey, HttpRequest> {
    private static final String url = "/recall/en";
    private int num = 500;

    public EnsureRequestBuilder() {
    }

    public EnsureRequestBuilder(Config config) {
    ...
    public CompletableFuture<Optional<HttpRequest>> apply(RetrieveKey retrieveKey) {
        ...
        if (retrieveKey.getKey() != null) {
            ...
            if(retrieveKey.getType().equals(RetrieverType.svrandom_en.name())){
