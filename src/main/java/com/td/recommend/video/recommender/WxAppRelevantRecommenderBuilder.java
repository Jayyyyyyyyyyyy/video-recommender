...
import com.td.recommend.video.rank.predictor.RelevantPredictor;
import com.td.recommend.video.rank.predictor.TalentWeightedPredictor;
...
import com.td.recommend.video.retriever.keybuilder.relevant.WxAppRelevantVideoRetrieveKeyBuilderFactory;
import com.td.recommend.video.retriever.validator.RetrieveValidator;

import java.util.Set;

public class WxAppRelevantRecommenderBuilder extends RecommenderBuilder<DocItem> {

    private int num = 550;
    public WxAppRelevantRecommenderBuilder(VideoRecommenderContext recommendContext) {
        super(recommendContext);
    }

    public WxAppRelevantRecommenderBuilder(VideoRecommenderContext recommendContext, int num) {
        super(recommendContext);
        this.num = num;
    }
    ...
    protected String buildName() {
        return "wx-relevant-recommender";
    }
    ...
    protected IRetrieveKeyBuilder buildRetrieveKeyBuilder() {
        ...
        return WxAppRelevantVideoRetrieveKeyBuilderFactory.getInstance().create(videoRecommendContext);
    }
    ...
    protected IPredictor buildPredictor() {
        ...
        if(buckets.contains("relevant_user_boost-yes")){
            return new TalentWeightedPredictor(predictor);
        }else {
            return predictor;
        }
    }
