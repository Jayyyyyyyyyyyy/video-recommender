...
d=$(date +%Y_%m_%d)
tail -n 10 ${TBASEDIR}/logs/jetty-${d}.request.log > /tmp/warmup.log
tail -n 40 ${TBASEDIR}/logs/dubbo_request.log > /tmp/warmup-dubbo.log
####### main ######

mkdir -p $TBASEDIR/var $TBASEDIR/logs $TBASEDIR/conf
